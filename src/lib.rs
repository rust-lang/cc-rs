...
//!    * When using `prefer_clang_cl_over_msvc`, make sure that the `C++ Clang compiler for Windows` component is installed through the Visual Studio Installer, so that `cc-rs` can find `clang-cl.exe`.
