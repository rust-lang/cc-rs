...
fn main() {
    ...
    let out_dir = PathBuf::from(
        env::var_os("GCCTEST_OUT_DIR").unwrap_or_else(|| panic!("{}: GCCTEST_OUT_DIR not found", program)),
    );
    ...
    for i in 0.. {
        ...
        let mut f = File::create(candidate).unwrap_or_else(|_| panic!("{}: can't create candidate: {}",
            program,
            candidate.to_string_lossy()));
        for arg in args {
            writeln!(f, "{}", arg).unwrap_or_else(|_| panic!("{}: can't write to candidate: {}",
                program,
                candidate.to_string_lossy()));
        }
        break;
    ...
    File::create(path).unwrap_or_else(|_| panic!("{}: can't create libfoo.a: {}",
        program,
