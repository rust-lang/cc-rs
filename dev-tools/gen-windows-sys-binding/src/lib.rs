...
//
// ```sh
// cargo run -p gen-windows-sys-binding
// ```
...
pub fn generate_bindings() {
    ...
    let bindings =
        fs::read_to_string(temp_file.path()).expect("failed to read temp windows_sys_bindings.rs");

    let mut f: BufWriter<fs::File> = fs::File::create(format!(
        "{MANIFEST_DIR}/../../find-msvc-tools/src/windows_sys_bindings.rs"
    ))
    .map(BufWriter::new)
    .expect("failed to create windows_sys_bindings.rs");
